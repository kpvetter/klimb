...
proc ::tkcon::InitUI {title} {
    ...
    ## Scrollbar
    set PRIV(scrolly) [::tk::scrollbar $w.sy -takefocus 0 -bd 1 \
...
proc ::tkcon::About {} {
	...
	::tk::button $w.b -text Dismiss -command [list wm withdraw $w]
...
proc ::tkcon::NamespacesList {names} {
    ...
    ::tk::scrollbar $f.scrollv -command [list $f.names yview]
    ::tk::scrollbar $f.scrollh -command [list $f.names xview] -orient horizontal
    frame $f.buttons
    ::tk::button $f.cancel -text "Cancel" -command [list destroy $f]
...
proc ::tkcon::FindBox {w {str {}}} {
	...
	::tk::checkbutton $base.opt.c -text "Case Sensitive" \
		-variable ::tkcon::PRIV(find,case)
	::tk::checkbutton $base.opt.r -text "Use Regexp" -variable ::tkcon::PRIV(find,reg)
	...
	::tk::button $base.btn.fnd -text "Find" -width 6
	::tk::button $base.btn.clr -text "Clear" -width 6
	::tk::button $base.btn.dis -text "Dismiss" -width 6
...
##
# KPV -- default to main, remove trailing "::"
proc ::tkcon::AttachNamespace { {name Main} } {
    variable PRIV
    variable OPT
...
proc ::tkcon::NewSocket {} {
	...
	::tk::button $t.ok -text "OK" -command {set ::tkcon::PRIV(grab) 1}
...
proc ::tkcon::MainInit {} {
	...
	if {[winfo exists $w]} {
	    ...
	    toplevel $w
	    frame $w.btn
	    ::tk::scrollbar $w.sy -takefocus 0 -bd 1 -command [list $w.text yview]
	    ...
	    ::tk::button $w.btn.close -text "Dismiss" -width 11 \
		    -command [list destroy $w]
	    ::tk::button $w.btn.check  -text "Recheckpoint" -width 11
	    ::tk::button $w.btn.revert -text "Revert" -width 11
	    ::tk::button $w.btn.expand -text "Verbose" -width 11
	    ::tk::button $w.btn.update -text "Update" -width 11
...
proc ::tkcon::Event {int {str {}}} {
	...
	if {$int > 0} {
		...
		if {[incr event] == $nextid} {
		    ...
		    if {$p > 0} continue ;# KPV -- only match at beginning of line
	    ...
	    # KPV: v2.3+ stops at the first match. Code below is from v2.2
...
proc tkcon {cmd args} {
	...
	get - gets {
		...
		::tk::scrollbar $t.sx -orient h -takefocus 0 -highlightthick 0 \
			-command [list $t.data xview]
		::tk::scrollbar $t.sy -orient v -takefocus 0 -highlightthick 0 \
...
proc edit {args} {
	...
	text $w.text -wrap none \
		...
		-font $::tkcon::OPT(font)
	::tk::scrollbar $w.sx -orient h -takefocus 0 -bd 1 \
		-command [list $w.text xview]
	::tk::scrollbar $w.sy -orient v -takefocus 0 -bd 1 \
...
proc ::tkcon::RetrieveAuthentication {} {
    ...
    set f1 [frame ${dlg}.f1]
    set f2 [frame ${dlg}.f2]
    ::tk::button $f2.b -text "OK" -command "destroy $dlg"
