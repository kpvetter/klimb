...
# KPV May 12, 2008 - tile enabled
# KPV Jun 09, 2008 - explicit ::ttk::<widget>

package require Tk
package require Ttk
package provide kswaplist 0.1.2
...
proc ::swaplist::swaplist {w var list1 list2 args} {
    ...
    parseOpts options {{-llabel {}} {-rlabel {}} {-title {}} -embed {-default {}} \
    ...
    frame $w.oc -pady 7
    ::ttk::button $w.oc.ok -default active -text "OK" -command {set ::swaplist::whichButton 1}
    ::ttk::button $w.oc.cancel -text "Cancel" -command {set ::swaplist::whichButton 0}
...
proc ::swaplist::createSwaplist {w var list1 list2 args} {
    ...
    parseOpts options {{-llabel {}} {-rlabel {}} {-title {}} {-default {}} \
    ...
    ::ttk::label $w.heading1 -text $options(-llabel) -anchor w
    ::ttk::label $w.heading2 -text $options(-rlabel) -anchor w

    foreach x {list1 list2} {
        ::ttk::frame $w.$x -class Border -relief sunken
        ::ttk::scrollbar $w.$x.scrolly -orient v -command [list $w.$x.list yview]
        ::ttk::scrollbar $w.$x.scrollx -orient h -command [list $w.$x.list xview]
        listbox $w.$x.list -selectmode extended -activestyle none \
	    -yscrollcommand [list $w.$x.scrolly set] \
	    -xscrollcommand [list $w.$x.scrollx set]
    ...
    ::ttk::frame $w.lr
    if {$options(-reorder)} {
        ::ttk::button $w.lr.right -width $width -text $options(-rbuttontext) -command [list ::swaplist::ShiftRNormal $w $olist]
    } else {
        ::ttk::button $w.lr.right -width $width -text $options(-rbuttontext) -command [list ::swaplist::ShiftRNoReorder $w $olist]
    }
    ::ttk::button $w.lr.left -width $width -text $options(-lbuttontext) -command [list ::swaplist::ShiftL $w $olist]
    ::ttk::button $w.default -command [list $olist $options(-default)]	;# NEVER GETS INVOKED!!!
    ...
    ::ttk::frame $w.ud
    ::ttk::button $w.ud.up   -width $width -text $options(-ubuttontext) -command [list ::swaplist::ShiftUD $w.list2.list u]
    ::ttk::button $w.ud.down -width $width -text $options(-dbuttontext) -command [list ::swaplist::ShiftUD $w.list2.list d]
...
proc ::swaplist::Reset {w} {
    foreach {list1 list2} [$w.default cget -command] break
    
    foreach x $list2 {				;# remove items in list2 from list1
        if {[set i [lsearch $list1 $x]] >= 0} {
            set list1 [lreplace $list1 $i $i]
        }
    }
    $w.list1.list delete 0 end
    $w.list2.list delete 0 end
    eval [list $w.list1.list insert end] $list1
    eval [list $w.list2.list insert end] $list2
}
