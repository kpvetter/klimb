...
if {[info exists ::scrolledframe::version]} { namespace delete ::scrolledframe }
...
namespace eval ::scrolledframe {
	...
	frame $w				;# Scrolled frame
	rename $w ::scrolledframe::_$w		;# Trap the reference
	interp alias {} $w {} ::scrolledframe::dispatch $w ;# redirect to dispatch
	frame $w.scrolled -highlightt 0 -padx 0 -pady 0	;# create scrollable internal frame
	place $w.scrolled -in $w -x 0 -y 0	;# place it
	if {$(debug,place)} { puts "place $w.scrolled -in $w -x 0 -y 0" }
	
	set ($w:vheight) 0			;# init internal data
	set ($w:vwidth) 0
	set ($w:vtop) 0
	set ($w:vleft) 0
	set ($w:xscroll) ""
    ...
    # resize proc
    #
    # Update the scrollbars if necessary, in response to a change in
    # either the viewing window or the scrolled object.
    # Replaces the old resize and the old vresize
    # A <Configure> call may mean any change to the viewing window or
    # the scrolled object. We only need to resize the scrollbars if
    # the size of one of these objects has changed. Usually the window
    # sizes have not changed, and so the proc will not resize the
    # scrollbars.
    # --------------
    # parm1: widget name
    ...
    proc ::scrolledframe::XYScrolledframe {W width height} {
	frame $W
	::scrolledframe::scrolledframe $W.sf -height $height -width $width \
	    -yscroll "$W.vs set" -xscroll "$W.hs set"
	scrollbar $W.vs -command "$W.sf yview"
	scrollbar $W.hs -command "$W.sf xview" -orient horizontal

	grid $W.sf -row 0 -column 0 -sticky news
	grid $W.vs -row 0 -column 1 -sticky nsw
	grid $W.hs -row 1 -column 0 -stick sew
